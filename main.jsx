...
function AdminLogin({onSuccess}){
 ...
 return <main className="container login"><div className="eyebrow">PRIVATE ADMIN</div><h1>Admin login</h1><p>Sign in to manage marketplace listings.</p><form onSubmit={e=>{e.preventDefault();if(pass==="admin123"){localStorage.setItem(ADMIN_KEY,"1");onSuccess()}else setErr("Incorrect password.")}}><input type="password" value={pass} onChange={e=>setPass(e.target.value)} placeholder="Admin password"/>{err&&<small className="err">{err}</small>}<button className="primary">Enter Dashboard</button></form></main>
}
